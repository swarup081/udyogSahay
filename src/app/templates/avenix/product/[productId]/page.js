        ...
        <div className="container mx-auto px-4 md:px-6 py-12 md:py-20 font-sans">
            <StaggerGrid className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-start">
                ...
                <StaggerItem className="relative group w-full max-w-lg mx-auto md:mx-0">
                    <div className="bg-brand-primary overflow-hidden rounded-xl md:rounded-lg relative aspect-[4/5] max-h-[60vh] md:max-h-[600px] w-full" onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
                        <img 
                ...
                <StaggerItem className="flex flex-col">
                    ...
                    <p className="text-[5vw] md:text-3xl font-light text-brand-secondary mb-8">
                        ₹{product.price} <span className="text-sm text-brand-text/50 font-normal">/ {product.unit}</span>
                    </p>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 mb-12">
                        <div className="flex items-center border border-brand-text/20 rounded-full w-full sm:w-auto justify-between sm:justify-start">
                            <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="w-12 h-12 text-2xl text-brand-text/70 hover:bg-brand-primary rounded-l-full flex items-center justify-center">-</button>
                            <span className="w-12 h-12 flex items-center justify-center text-lg font-bold border-x border-brand-text/20">{quantity}</span>
                            <button onClick={() => setQuantity(q => Math.min(stock, q + 1))} className="w-12 h-12 text-2xl text-brand-text/70 hover:bg-brand-primary rounded-r-full flex items-center justify-center">+</button>
                        </div>
                        <button 
                            onClick={handleAddToCart}
                            disabled={isOutOfStock}
                            className="flex-grow h-12 bg-brand-secondary text-brand-bg font-medium text-base uppercase tracking-wider hover:opacity-80 transition-opacity rounded-full disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                        >
            ...
            {relatedProducts.length > 0 && (
                <ScrollSection direction="up" className="mt-12 md:mt-24 pt-8 md:pt-16 border-t border-brand-text/10">
                    ...
                    <StaggerGrid className="grid grid-cols-2 md:grid-cols-4 gap-x-4 md:gap-x-8 gap-y-8 md:gap-y-16 items-stretch">
                         {relatedProducts.map(item => (
                             <StaggerItem key={item.id}>
                                 <ProductCard item={item} templateName="avenix" />
                             </StaggerItem>
                         ))}
                    </StaggerGrid>
                </ScrollSection>
